import React, {useState, useEffect, useContext} from 'react'
import {useMessage} from '../hooks/message.hook'
import {useHttp} from "../hooks/http.hook";
import {AuthContext} from "../context/AuthContext";
import {Form, Button, Modal, Card} from 'react-bootstrap'
import photo from '../img/default-thumbnail.jpg'



export const Profile = ({ data }) => {

    let imageBase64Code

    const [show, setShow] = useState(false);

    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);
    const {token} = useContext(AuthContext)

    const deletePhotoHandler = async () => {
        try{
            const del_photo = await request('/api/auth/delete-user-photo', 'PATCH', null, {
                Authorization: `Bearer ${token}`
            })
            setShow(false)
            document.location.reload()

        }catch (e) {
        }
    }

    const changeHandler = () => {
        let filesSelected = document.getElementById("exampleFormControlFile1").files;
        if (filesSelected.length > 0) {
            let fileToLoad = filesSelected[0];

            let fileReader = new FileReader();

            fileReader.onload = function(fileLoadedEvent) {
                let srcData = fileLoadedEvent.target.result; // <--- data: base64

                imageBase64Code = srcData
                let user_img = document.getElementById('user_img')
                user_img.src = srcData

                let save_profile_img = document.getElementById('save_profile_img')
                save_profile_img.style.display = 'block'

            }
            fileReader.readAsDataURL(fileToLoad);


        }
    }
    const savePhotoHandler = async () => {
        try{
            let body = {
                'user_img': imageBase64Code
            }
            const savedName = await request(`/api/auth/save-user-photo`, 'PATCH', body,{
                Authorization: `Bearer ${auth.token}`
            })
            document.location.reload()
        }catch (e) {
        }
    }
    let team = data.hasTeam
    let hasTeam = "да"
    if(!team){
        hasTeam = "нет"
    }
    const changeUserName = () => {
      let change_user_name = document.getElementById('change_user_name')
      let input_user_name = document.getElementById('input_user_name')
      let save_user_name = document.getElementById('save_user_name')

        change_user_name.style.display = 'none'
        input_user_name.style.display = 'block'
        save_user_name.style.display = 'block'

        let change_user_surname = document.getElementById('change_user_surname')
        
        change_user_surname.style.display = 'none'
    }
    const changeUserSurname = () => {
        let change_user_surname = document.getElementById('change_user_surname')
        let input_user_surname = document.getElementById('input_user_surname')
        let save_user_surname = document.getElementById('save_user_surname')
  
        change_user_surname.style.display = 'none'
        input_user_surname.style.display = 'block'
        save_user_surname.style.display = 'block'

        let change_user_name = document.getElementById('change_user_name')
        
        change_user_name.style.display = 'none'
    }
    
    const auth = useContext(AuthContext)

    const message = useMessage()
    const {loading, error, request, clearError} = useHttp()

    const [name, setName] = useState({
        user_name: ''
    })

    const [surname, setSurname] = useState({
        surname: ''
    })

    const changeHandlerName = event => {
        setName({ ...name, [event.target.name]: event.target.value })
    }

    const changeHandlerSurname = event => {
        setSurname({ ...surname, [event.target.name]: event.target.value })
    }

    const saveNameHandler = async () => {
        try{
            const savedName = await request('/api/auth/update-user-name', 'PATCH', {...name},{
                Authorization: `Bearer ${auth.token}`
            })       
        }catch (e) {
        }
    }

    const saveSurnameHandler = async () => {
        try{
            const savedSurname = await request('/api/auth/update-user-surname', 'PATCH', {...surname},{
                Authorization: `Bearer ${auth.token}`
            })       
        }catch (e) {
        }
    }

    useEffect(() => {
        message(error)
        clearError()
    }, [error, message, clearError])

    useEffect(() => {
        window.M.updateTextFields()
    }, [])
    
    // if(loading){
    //     return document.location.reload()
    // }

    let no_photo = document.getElementById('no_photo')
    let edit_button_user_data = document.getElementById('edit_button_user_data')
    let exampleFormControlFile1 = document.getElementById('exampleFormControlFile1')
    let user_img = document.getElementById('user_img')


    if(data.user_img == " "){

        no_photo.style.display = 'block'
        user_img.src = "data:image/jpeg;base64,"

        if(edit_button_user_data.style.display == "none"){
            exampleFormControlFile1.style.display = "block"
        }

    }








        
  
    return(
        <>
            <Modal
                show={show}
                onHide={handleClose}
                backdrop="static"
                keyboard={false}
            >
                <Modal.Header closeButton>
                    <Modal.Title>Удалить фото</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    Вы дейставительно хотите это сделать ?
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={handleClose}>
                        Отменить
                    </Button>
                    <Button variant="primary" onClick={deletePhotoHandler}>Да</Button>
                </Modal.Footer>
            </Modal>

            <div className="row">
                <div className="col-xl-5 mb-5" style={{ textAlign: 'center'}}>
                    <Card>
                        <Card.Img id="user_img" src={data.user_img} alt="" style={{alignItems: "center"}}/>
                        <Card.Text id="no_photo" style={{textAlign: 'center', display: 'none'}}>Фото пока нет</Card.Text>
                    </Card>
                </div>
                <div className="col-xl-7">
                <Form.Group>
                    <div style={{display: 'flex', margin: 0}}>
                        <p style={{marginRight: '50px', fontSize: '20px', marginBottom: 0}}>Имя: {data.user_name}</p>
                        <Button id="change_user_name" onClick={changeUserName}>изменить</Button>
                        <Form.Control
                            id="input_user_name"
                            size="sm"
                            type="text"
                            placeholder="Введите имя"
                            name="user_name"
                            onChange={changeHandlerName}
                        />
                        <Button
                            id="save_user_name"
                            onClick={saveNameHandler}
                            disabled={loading}
                        >
                            сохранить
                        </Button>
                    </div>

                </Form.Group>
                <Form.Group>
                    <div style={{display: 'flex', margin: 0}}>
                        <p style={{marginRight: '50px', fontSize: '20px'}}>Фамилия: {data.surname}</p>
                        <Button id="change_user_surname" onClick={changeUserSurname}>Изменить</Button>
                        <Form.Control
                            id="input_user_surname"
                            size="sm"
                            type="text"
                            placeholder="Введите фамилию"
                            name="surname"
                            onChange={changeHandlerSurname}
                        />
                        <Button
                            id="save_user_surname"
                            onClick={saveSurnameHandler}
                            disabled={loading}
                        >
                            сохранить
                        </Button>
                    </div>

                </Form.Group>
                    <p style={{marginRight: '50px', fontSize: '20px'}}>Дата регистрации: {data.regDateTime}</p>
                    <p style={{marginRight: '50px', fontSize: '20px'}}>Ваша команда: {hasTeam}</p>
                    <p style={{marginRight: '50px', fontSize: '20px'}}>Статус: {data.role}</p>

                <Form>
                        <Form.Group>
                            <Form.File id="exampleFormControlFile1" onChange={changeHandler}/>
                        </Form.Group>

                    <Button
                        id="delete_user_img"
                        onClick={handleShow}
                    >
                        Удалить фото
                    </Button>

                        <Button
                            id="save_profile_img"
                            onClick={savePhotoHandler}
                            disabled={loading}
                            style={{display:'none'}}
                        >
                            сохранить фото профиля
                        </Button>

                </Form>
                </div>
            </div>
        </>
    )
}