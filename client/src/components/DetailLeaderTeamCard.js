import React, {useContext, useState} from "react";
import {Button, Card, Table, Form, Modal} from "react-bootstrap";
import {useHttp} from "../hooks/http.hook";
import {Loader} from "./Loader";
import {AuthContext} from "../context/AuthContext";
import {useHistory, useParams} from "react-router-dom";

export const DetailLeaderTeamCard = ({ data }) => {
    const {loading, request} = useHttp()
    const {token} = useContext(AuthContext)
    const teamId = useParams().id
    const [show, setShow] = useState(false);
    const [photo, setPhoto] = useState(false)

    const handleClose = () => setShow(false);
    const handleShow = () => setShow(true);


    const patchHandler = () => {
        try {

            let filesSelected = document.getElementById("addTeamPhInput").files;


            if (filesSelected.length > 0) {
                let fileToLoad = filesSelected[0];

                let fileReader = new FileReader();
                fileReader.readAsDataURL(fileToLoad);
                fileReader.onload = async function (fileLoadedEvent) {
                    let srcData = fileLoadedEvent.target.result; // <--- data: base64
                    let body = {
                        'img': srcData
                    }
                    const fetched = await request(`/api/team/update-team-photo?id=${teamId}`, 'PATCH', body, {
                        Authorization: `Bearer ${token}`
                    })
                    document.location.reload()
                }
            }
        }
        catch (e) {
            
        }
    }
    const photoHandler = () => {
        setPhoto(true)
        let filesSelected = document.getElementById("addTeamPhInput").files;
        if (filesSelected.length > 0) {
            let fileToLoad = filesSelected[0];

            let fileReader = new FileReader();
            fileReader.readAsDataURL(fileToLoad);
            fileReader.onload = async function (fileLoadedEvent) {
                let srcData = fileLoadedEvent.target.result; // <--- data: base64
                let teamLeaderIMG = document.getElementById('teamLeaderIMG')
                teamLeaderIMG.src = srcData

            }
        }

    }
    
    const delPhHandler = async () => {
        try{
            setShow(false)
            const fetched = await request(`/api/team/delete-team-photo?id=${teamId}`, 'PATCH', null, {
                Authorization: `Bearer ${token}`
            })
            document.location.reload()
        }
        catch (e) {

        }
    }

    if(loading){
        return <Loader/>
    }
    let image = "data:image/jpeg;base64,"

    return(
        <>
            <Modal
                show={show}
                onHide={handleClose}
                backdrop="static"
                keyboard={false}
            >
                <Modal.Header closeButton>
                </Modal.Header>
                <Modal.Body>
                    Вы действительно хотите удалить фото?
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="danger" onClick={delPhHandler}>
                        Да
                    </Button>
                    <Button variant="success" onClick={handleClose}>Нет</Button>
                </Modal.Footer>
            </Modal>
            <div className="row mt-5">
                <div className="col-xl-3">
                    <Card.Img id="teamLeaderIMG" className="mb-2" src={data.img == "" ? image : data.img}/>

                    <Button id="add_ph_team" className="mb-3" style={{display: `${data.img == "" ? 'block':'none'}`, width: '100%'}} onClick={() => {
                        let addTeamPh = document.getElementById('addTeamPh')
                        let cancelAddTeamPh = document.getElementById('cancelAddTeamPh')
                        let add_ph_team = document.getElementById('add_ph_team')
                        addTeamPh.style.display = 'block'
                        cancelAddTeamPh.style.display = 'block'
                        add_ph_team.style.display = 'none'
                    }}>Добавить фото профиля</Button>

                    <Form.Group id="addTeamPh" style={{display: 'none'}} controlId="formFileMultiple" className="mb-3">
                        <Form.Control onChange={photoHandler} id="addTeamPhInput" type="file" multiple />
                    </Form.Group>

                    <Button id="successAddTeamPh"
                            style={{
                                display: `${photo ? 'block':'none'}`,

                                width: '100%'}} className="mb-2" variant="success" onClick={patchHandler}>Ок</Button>

                    <Button id="cancelAddTeamPh" variant="danger" style={{display: 'none', width: '100%'}} onClick={() => {
                        let addTeamPh = document.getElementById('addTeamPh')
                        let cancelAddTeamPh = document.getElementById('cancelAddTeamPh')
                        let successAddTeamPh = document.getElementById('successAddTeamPh')
                        let add_ph_team = document.getElementById('add_ph_team')
                        addTeamPh.style.display = 'none'
                        cancelAddTeamPh.style.display = 'none'
                        successAddTeamPh.style.display = 'none'
                        add_ph_team.style.display = 'block'
                        document.getElementById('addTeamPhInput').value = ""
                    }}>Отмена</Button>
                    <Button variant="warning" style={{display: `${data.img == "" ? 'none':'block'}`,width: '100%'}} onClick={handleShow}>Удалить фото</Button>
                </div>
                <div className="col-xl-9" border="primary">
                    <Card.Header as="h5">Название: {data.team_name}</Card.Header>
                    <Card.Body>
                        <Card.Text>
                            Описание: {data.description}
                        </Card.Text>
                    </Card.Body>
                </div>
            </div>

            <hr/>


            <Card className="mt-5">
                <Card.Body>Игроки команды</Card.Body>
            </Card>


            <Table striped bordered hover variant="dark">
                <thead>
                <tr>
                    <th>#</th>
                    <th>Имя</th>
                    <th>Фамилия</th>
                    <th>Удалить</th>
                </tr>
                </thead>
                <tbody>
                {

                    data.users.map((user, index) => {
                        index +=1
                        return(
                            <tr>
                                <td>{index++}</td>
                                <td>{user.user_name}</td>
                                <td>{user.surname}</td>
                                <td><Button onClick={async () => {
                                    const fetched = await request(`/api/sensors/delete-user-by-leader?id=${data._id}`, 'DELETE', null, {
                                        Authorization: `Bearer ${token}`,
                                        'userId': user._id
                                    })
                                    document.location.reload()
                                }}>Удалить</Button></td>
                            </tr>
                        )
                    })

                }
                </tbody>
            </Table>

        </>
    )
}