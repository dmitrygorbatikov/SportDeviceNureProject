...
export const TeamsArray = ({ data }) => {
    ...
    let create_team = document.getElementById('create_team')

    if(userData.role == "leader"){
        create_team.style.display = "block"
    }
    ...
    if(data.length == 0){
        return(
            <Card body style={{textAlign: 'center'}}>По вашему запросу ничего не найдено</Card>
        )
    }
    let link
    ...
    return(
        <>
        {
            data.map(d => {
                ...
                link = `/detail-team/${d._id}`
                if(userId == d.leader){
                    link = `/detail-leader-team/${d._id}`
                }
                if(d.users != 0){
                    d.users.map(user =>{
                        if(user._id == userId){
                            link = `detail-user-team/${d._id}`
                        }
                    })
                }
                ...
                <>
                    <div className="row">
                        <div className="col-xl-3">
                            <Card.Img src={d.img == "" ? image : d.img}/>
                        </div>
                        <div className="col-xl-9" border="primary">
                            <Card.Header as="h5">{d.team_name}</Card.Header>
                            <Card.Body>
                                <Card.Text>
                                    {d.description}
                                </Card.Text>
                                <div className="d-flex">
                                    <Button variant="primary" className="check_team"><a href={link}>Просмотр команды</a></Button>
